export default (items) => {
    return {
        ...
        getTerms() {
            ...
            // Check for pipe character in the array
            for (let i = 0; i < this.terms.length; i++) {
                const term = this.terms[i];
                if (term && term.includes('|')) {
                    const splitTerms = term.split('|');
                    this.terms.splice(i, 1, ...splitTerms);
                    i += splitTerms.length - 1; // Adjust the loop index
                }
            }
        ...
        filteredItems() {
            ...
            if (this.selectedTerm !== null) {
                filteredItems = filteredItems.filter(item => {
                    return item.terms.includes(this.selectedTerm);
                });
            }
            ...
            if (this.startDate !== '' && this.endDate !== '') {
                filteredItems = filteredItems.filter(item => {
                    const itemDate = new Date(item.when_published);
